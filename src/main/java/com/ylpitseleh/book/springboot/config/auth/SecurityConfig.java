package com.ylpitseleh.book.springboot.config.auth;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                .csrf().disable() //h2-console화면을 사용하기 위해 해당 옵션들을 disable한다.
                ...
                .anyRequest().authenticated() //"/"등 지정된 URL들은 permitAll()옵션을 토앻 전체 열람 권한을 주었다.
                .and() // "/api/v1/**" 주소를 가진 API는 USER 권한을 가진 사람만 가능하도록 했다.
                .logout() //anyRequest : 설정된 값들 이외에 나머지 URL들을 나타냄. 여기서는 autheticated()을 추가하여 나머지 URL들은 모두 인증된 사용자들에게만 허용하게 한다.
